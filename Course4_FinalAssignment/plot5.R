...
# 4th: create subset for Baltimore City, Maryland (fips == "24510") motor vehicle emissions
motorVehicleSubset <- filter( NEI_df, fips == "24510" & SCC %in% motorVeicles$SCC )

# 5th: sum PM2.5 per year
total_PM25_per_year <- with( motorVehicleSubset, aggregate( x=Emissions, by=list(year), FUN=sum ))

# 6th:: plot totals (y-axis) by year (x-axis)
plot( total_PM25_per_year$Group.1, total_PM25_per_year$x,
      type="b", # both points and lines
      xlab="Year",
      ylab="Total PM2.5 per year from Motor Vehicles",
      main="Baltimore, Maryland" )

# 7th: Save to plot5.png
