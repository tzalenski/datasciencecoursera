...
# 3rd: create subset for just Baltimore City, Maryland (fips == "24510")
baltimore <- filter( NEI, fips == "24510" )

# 4th: sum PM2.5 from each type per year
total_PM25_per_year_by_type <- with( baltimore, aggregate( x=Emissions, by=list(type,year), FUN=sum ))

# 5th:: plot totals (y-axis) by year (x-axis)
ggplot( data=total_PM25_per_year_by_type, aes(x = Group.2, y = x, group=Group.1)) +
