...
# 3rd: Find subset of classification codes that use coal
usesCoal <- filter( SCC_df, grepl( "Coal", EI.Sector))

# 4th: create subset of emmisions data that uses Coal
coalSubset <- filter( NEI_df, SCC %in% usesCoal$SCC )

# 5th: sum PM2.5 per year
total_PM25_per_year_use_coal <- with( coalSubset, aggregate( x=Emissions, by=list(year), FUN=sum ))

# 6th:: plot totals (y-axis) by year (x-axis)
plot( total_PM25_per_year_use_coal$Group.1, total_PM25_per_year_use_coal$x,
      type="b", # both points and lines
      xlab="Year",
      ylab="Total PM2.5 from Coal Use per year",
      main="All U.S. Counties" )

# 7th: Save to plot4.png
